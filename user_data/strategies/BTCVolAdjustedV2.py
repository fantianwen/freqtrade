...
class TechnicalIndicators:
    ...
    def calculate_adx(df, period=14):
        ...
        return (
            adx.fillna(25).replace([np.inf, -np.inf], 25),
...
class BTCVolAdjustedV2(IStrategy):
    ...
    # Entry thresholds
    entry_pred_threshold = DecimalParameter(
        0.3,
        3.0,
        default=1.0,
        decimals=2,
    ...
    entry_range_threshold = DecimalParameter(
        0.3,
        3.0,
        default=1.0,
        decimals=2,
    ...
    entry_vzo_threshold = IntParameter(
        1,
        5,
        default=3,
        space="buy",
    ...
    # Exit thresholds
    exit_sideways_threshold = DecimalParameter(
        0.3,
        3.0,
        default=1.0,
        decimals=2,
    ...
    exit_flip_threshold = DecimalParameter(
        0.3,
        3.0,
        default=1.0,
        decimals=2,
    ...
    exit_sideways_count = IntParameter(
        2,
        5,
        default=3,
        space="sell",
    ...
    # Position sizing
    highvol_size_mult = DecimalParameter(
        0.2,
        1.0,
        default=0.5,
        decimals=2,
    ...
    boundary_4h_boost = DecimalParameter(
        1.0,
        1.5,
        default=1.25,
        decimals=2,
    ...
    confidence_scale = DecimalParameter(
        1.0,
        4.0,
        default=2.0,
        decimals=1,
    ...
    confidence_floor = DecimalParameter(
        0.1,
        0.8,
        default=0.3,
        decimals=2,
    ...
    # Stoploss
    sl_min_price_pct = DecimalParameter(
        0.5,
        3.0,
        default=1.0,
        decimals=1,
    ...
    _prediction_cache: Dict[
        str, dict
    ] = {}  # candle_date_str -> {ml_direction, ml_vol_regression, ...}
        ...
        for name, spec in model_specs.items():
            ...
            if not os.path.isabs(path):
                ...
                candidates = [
                    path,  # as-is (cwd)
                    os.path.join(
                        config.get("user_data_dir", ""), "..", path
                    ),  # relative to userdir parent
                ]
                # Also try relative to common freqtrade roots
                for base in [
                    os.getcwd(),
                    os.path.dirname(os.path.abspath(__file__)),
                    "/home/ubuntu/alex-trading",
                ]:
                ...
                if resolved:
                    ...
                    logger.warning(
                        f"Model '{name}' not found. Tried: {[os.path.normpath(c) for c in candidates[:4]]}"
                    )
                    continue
            ...
            try:
                ...
                logger.info(
                    f"Loaded {name}: {type(self._models[name]['model']).__name__}, "
                    f"{n_feat} features"
                )
    ...
    # ================================================================
    # Apply cleaning params from training (leak-free inference)
    # ================================================================

    @staticmethod
    def _apply_cleaning_params(X: DataFrame, params: dict) -> DataFrame:
        """
        Apply pre-computed cleaning parameters from training to inference data.
        Mirrors the apply_cleaning_params() function from the training pipeline.
        Handles the case where params is None (old models without cleaning_params).
        """
        if params is None:
            return X
        ...
        # Drop high-NaN columns (identified from training)
        drop_cols = [c for c in params.get("high_nan_cols", []) if c in X.columns]
        if drop_cols:
        ...
        for col in X.columns:
            if X[col].isna().any():
                med = params.get("col_medians", {}).get(col, 0)
                X[col] = X[col].fillna(med)

        # Clip outliers using training percentiles
        for col in X.columns:
            if col in params.get("col_clips", {}):
                lo, hi = params["col_clips"][col]
                X[col] = X[col].clip(lo, hi)

        # Drop constant columns (identified from training)
        const_cols = [c for c in params.get("constant_cols", []) if c in X.columns]
        if const_cols:
    ...
    def _extract_tf_features(self, df: DataFrame, tf: str) -> dict:
        ...
        return {
            ...
            f"{p}_kdj_d": d,
            f"{p}_kdj_j": j,
            f"{p}_kdj_golden": kg.astype(int).fillna(0),
            f"{p}_kdj_death": kd.astype(int).fillna(0),
            f"{p}_macd": macd,
            f"{p}_macd_signal": sig,
            f"{p}_macd_hist": hist,
            f"{p}_macd_golden": mg.astype(int).fillna(0),
            f"{p}_macd_death": md.astype(int).fillna(0),
            f"{p}_volatility": vol_ann.fillna(0),
            f"{p}_vol_ratio_ma5": vr5,
            f"{p}_vol_ratio_ma10": vr10,
            f"{p}_vol_ratio_ma20": vr20,
            f"{p}_vol_change_1": vc1,
            f"{p}_vol_change_5": vc5,
            f"{p}_vol_trend": vt,
            f"{p}_vol_position": vpos,
            f"{p}_vol_spike": vspk,
            f"{p}_vol_shrink": vshr,
            f"{p}_vol_price_divergence": vpd,
            f"{p}_price_position": ppos,
            f"{p}_trend_strength": ts,
            f"{p}_rsi_7": rsi7,
            f"{p}_rsi_14": rsi14,
            f"{p}_rsi_21": rsi21,
            f"{p}_rsi_overbought": rob,
            f"{p}_rsi_oversold": ros,
            f"{p}_rsi_trend": rtr,
            f"{p}_roc_5": roc5,
            f"{p}_roc_10": roc10,
            f"{p}_roc_20": roc20,
            ...
            f"{p}_williams_r": wr,
            f"{p}_cci": cci,
            f"{p}_cci_overbought": ccob,
            f"{p}_cci_oversold": ccos,
            f"{p}_adx": adx,
            f"{p}_plus_di": pdi,
            f"{p}_minus_di": mdi,
            f"{p}_adx_strong_trend": adxs,
            f"{p}_adx_weak_trend": adxw,
            f"{p}_trend_bullish": trbull,
            f"{p}_stoch_rsi_k": srk,
            f"{p}_stoch_rsi_d": srd,
            f"{p}_vzo": vzo,
            f"{p}_vzo_ma": vzo_ma,
            f"{p}_vzo_slope": slope,
            f"{p}_vzo_zscore": vz,
            f"{p}_slope_zscore": sz,
            f"{p}_slope_accel": saccel,
            f"{p}_vzo_zone": vzn,
            f"{p}_slope_sign": ssn,
        }
    ...
    def _add_vol_specific_features(
        self, feat: dict, df_1h: DataFrame, df_dict: Dict[str, DataFrame]
    ) -> dict:
        ...
        feat["vol_ratio_6_24"] = (
            (vol6 / vol24.replace(0, 1e-8)).fillna(1).replace([np.inf, -np.inf], 1)
        )
        ...
        feat["atr_expansion"] = (
            (atr7 / atr20.replace(0, 1e-8)).fillna(1).replace([np.inf, -np.inf], 1)
        )
    ...
    def _single_window_core(
        ...
        if len(avail) >= 2:
            ...
            sign_df = pd.DataFrame(
                {t: all_feat[f"{t}_slope_sign"] for t in avail}, index=df_1h.index
            )
            all_feat["vzo_multi_tf_bullish"] = (zone_df > 0).sum(axis=1)
            all_feat["vzo_multi_tf_bearish"] = (zone_df < 0).sum(axis=1)
            all_feat["vzo_multi_tf_consensus"] = (
                all_feat["vzo_multi_tf_bullish"] - all_feat["vzo_multi_tf_bearish"]
            )
            all_feat["slope_multi_tf_bullish"] = (sign_df > 0).sum(axis=1)
            all_feat["slope_multi_tf_bearish"] = (sign_df < 0).sum(axis=1)
            all_feat["slope_multi_tf_consensus"] = (
                all_feat["slope_multi_tf_bullish"] - all_feat["slope_multi_tf_bearish"]
            )

            stfs = [t for t in ["5m", "15m"] if f"{t}_vzo" in all_feat]
            ltfs = [t for t in ["1h", "4h"] if f"{t}_vzo" in all_feat]
            if stfs and ltfs:
                all_feat["vzo_short_long_diff"] = pd.DataFrame(
                    {t: all_feat[f"{t}_vzo"] for t in stfs}, index=df_1h.index
                ).mean(axis=1) - pd.DataFrame(
                    {t: all_feat[f"{t}_vzo"] for t in ltfs}, index=df_1h.index
                ).mean(axis=1)
                all_feat["slope_short_long_diff"] = pd.DataFrame(
                    {t: all_feat[f"{t}_vzo_slope"] for t in stfs}, index=df_1h.index
                ).mean(axis=1) - pd.DataFrame(
                    {t: all_feat[f"{t}_vzo_slope"] for t in ltfs}, index=df_1h.index
                ).mean(axis=1)
        ...
        feat_df["signal_strength"] = (
            feat_df["multi_tf_golden_count"] - feat_df["multi_tf_death_count"]
        )
        ...
        df_1h["ml_vol_class"] = df_1h.get(
            "ml_vol_classifier", pd.Series(0, index=df_1h.index)
        ).astype(int)

        # VZO consensus
        df_1h["ml_vzo_consensus"] = feat_df.get(
            "vzo_multi_tf_consensus", pd.Series(0, index=df_1h.index)
        )
    ...
    def _build_features_and_predict(
        ...
        # Fix: only accept the FIRST prediction for each candle date.
        CACHE_COLS = [
            "ml_direction",
            "ml_vol_regression",
            "ml_range_regression",
        ...
        if len(df_1h) > 0:
            ...
            new_cached = 0
            for idx, dt_str in dates.items():
                if dt_str not in self._prediction_cache:
                    self._prediction_cache[dt_str] = {
                        col: float(df_1h.at[idx, col]) for col in CACHE_COLS if col in df_1h.columns
                    }
                    new_cached += 1
            ...
            if new_cached > 0:
                logger.info(
                    f"Cached {new_cached} new predictions. Latest @ {last_dt_str}: "
                    f"pred={self._prediction_cache[last_dt_str].get('ml_pred_pct', 0):.4f}, "
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Log latest predictions
        if len(dataframe) > 0:
            last = dataframe.iloc[-1]
            logger.info(
                f"Indicators done. Models loaded: {list(self._models.keys())}. "
                f"Latest predictions @ {last.get('date', 'N/A')}: "
                f"pred_pct={last.get('ml_pred_pct', 0):.4f}, "
                f"range_pct={last.get('ml_range_pct', 0):.4f}, "
                f"vol_class={int(last.get('ml_vol_class', 0))}, "
                f"vzo_consensus={int(last.get('ml_vzo_consensus', 0))}, "
                f"is_4h={int(last.get('ml_is_4h', 0))}"
            )
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        pt = self.entry_pred_threshold.value
        rt = self.entry_range_threshold.value
        vt = self.entry_vzo_threshold.value

        # LONG: strong direction + meaningful range + strong VZO alignment
        dataframe.loc[
            (pred > pt) & (rng > rt) & (vzo_c >= vt) & (dataframe["volume"] > 0), "enter_long"
        ] = 1

        # SHORT: strong direction + meaningful range + strong VZO alignment
        dataframe.loc[
            (pred < -pt) & (rng > rt) & (vzo_c <= -vt) & (dataframe["volume"] > 0), "enter_short"
        ] = 1

        nl = dataframe.get("enter_long", pd.Series(0)).sum()
        ns = dataframe.get("enter_short", pd.Series(0)).sum()

        # Log latest candle's entry evaluation
        if len(dataframe) > 0:
            last = dataframe.iloc[-1]
            p = float(last.get("ml_pred_pct", 0))
            r = float(last.get("ml_range_pct", 0))
            v = int(last.get("ml_vzo_consensus", 0))
            long_ok = p > pt and r > rt and v >= vt
            short_ok = p < -pt and r > rt and v <= -vt
            signal = "LONG" if long_ok else ("SHORT" if short_ok else "NONE")

            # Show which conditions pass/fail
            logger.info(
                f"Entry eval @ {last.get('date', 'N/A')}: "
                f"pred={p:+.3f} ({'Y' if abs(p) > pt else 'N'}>{pt}), "
                f"range={r:.3f} ({'Y' if r > rt else 'N'}>{rt}), "
                f"vzo={v} ({'Y' if (v >= vt or v <= -vt) else 'N'}>={vt}/≤-{vt}) "
                f"-> {signal}"
            )

        logger.info(f"Entry signals total: LONG={nl}, SHORT={ns}")
        return dataframe
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        pred = dataframe["ml_pred_pct"]
        st = self.exit_sideways_threshold.value
        ft = self.exit_flip_threshold.value
        sc = self.exit_sideways_count.value

        # Build consecutive sideways condition dynamically
        sideways_now = pred.abs() < st
        consecutive_sideways = sideways_now
        for i in range(1, sc):
            prev_col = f"ml_pred_pct_prev{i}"
            if prev_col in dataframe.columns:
                consecutive_sideways = consecutive_sideways & (dataframe[prev_col].abs() < st)

        # Exit LONG when direction flips strongly bearish
        # OR N consecutive sideways candles (conviction lost)
        dataframe.loc[
            ((pred < -ft) | consecutive_sideways) & (dataframe["volume"] > 0), "exit_long"
        ] = 1

        # Exit SHORT when direction flips strongly bullish
        # OR N consecutive sideways candles
        dataframe.loc[
            ((pred > ft) | consecutive_sideways) & (dataframe["volume"] > 0), "exit_short"
        ] = 1

        nel = dataframe.get("exit_long", pd.Series(0)).sum()
        nes = dataframe.get("exit_short", pd.Series(0)).sum()

        # Log latest candle's exit evaluation
        if len(dataframe) > 0:
            last = dataframe.iloc[-1]
            p = float(last.get("ml_pred_pct", 0))
            sw_now = abs(p) < st
            ...
            for i in range(1, sc):
                prev_col = f"ml_pred_pct_prev{i}"
                pv = float(last.get(prev_col, 0))
                all_sw = all_sw and abs(pv) < st
                preds_str = f"{pv:+.2f}->{preds_str}"
            ...
            reasons = []
            if flip_bear:
                reasons.append("bearish flip")
            if flip_bull:
                reasons.append("bullish flip")
            if all_sw:
                reasons.append(f"{sc}x sideways ({preds_str})")

            exit_long = flip_bear or all_sw
            exit_short = flip_bull or all_sw

            logger.info(
                f"Exit eval @ {last.get('date', 'N/A')}: "
                f"pred={p:+.3f} | "
                f"{'EXIT LONG' if exit_long else 'HOLD LONG'} | "
                f"{'EXIT SHORT' if exit_short else 'HOLD SHORT'}"
                f"{' | ' + ', '.join(reasons) if reasons else ''}"
            )

        logger.info(f"Exit signals total: LONG={nel}, SHORT={nes}")
        return dataframe
    ...
    def confirm_trade_entry(
        ...
        **kwargs,
        ...
        """
        Re-verify the cached prediction before placing the order.
        In live mode, sub-timeframe data may be incomplete when
        populate_indicators first runs, producing inflated predictions.
        ...
        if side == "long":
            ok = pred > pt and rng > rt and vzo >= vt
        else:
            ok = pred < -pt and rng > rt and vzo <= -vt

        if not ok:
            logger.warning(
                f"confirm_trade_entry REJECTED {side.upper()} @ {last.get('date', 'N/A')}: "
                f"pred={pred:+.4f}, range={rng:.4f}, vzo={vzo} — "
                f"thresholds: pred>{pt}, range>{rt}, vzo>={vt}/≤-{vt}"
            )
            return False

        logger.info(
            f"confirm_trade_entry CONFIRMED {side.upper()} @ {last.get('date', 'N/A')}: "
            f"pred={pred:+.4f}, range={rng:.4f}, vzo={vzo}"
        )
        return True
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        # Clamp: floor at hard stoploss, ceiling so at least sl_min_price_pct% price move
        sl = max(sl, self.stoploss)  # no wider than hard stoploss
        sl = min(sl, -(self.sl_min_price_pct.value / 100.0) * lev)  # no tighter than X% price move
        return sl
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        if vol_class == 1:
            base *= self.highvol_size_mult.value

        # Boost at 4h boundary
        is_4h = int(last.get("ml_is_4h", 0))
        if is_4h == 1:
            base *= self.boundary_4h_boost.value

        # Scale by prediction confidence (how far from 0)
        pred_pct = abs(float(last.get("ml_pred_pct", 0)))
        confidence_factor = min(pred_pct / self.confidence_scale.value, 1.0)
        base *= max(confidence_factor, self.confidence_floor.value)  # floor at configured min
